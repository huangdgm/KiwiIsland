    ...
    private void setUpOccupants(Scanner input) {
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            if (occType.equals("T")) {
                ...
                occupant = new Tool(occPos, occName, occDesc, weight, size, scanWikiDescription(input));
            } else if (occType.equals("E")) {
                double weight = input.nextDouble();
                double size = input.nextDouble();
                double energy = input.nextDouble();
                occupant = new Food(occPos, occName, occDesc, weight, size, energy, scanWikiDescription(input));
            } else if (occType.equals("H")) {
                double impact = input.nextDouble();
                occupant = new Hazard(occPos, occName, occDesc, impact, scanWikiDescription(input));
            } else if (occType.equals("K")) {
                occupant = new Kiwi(occPos, occName, occDesc, scanWikiDescription(input));
                totalKiwis++;
            } else if (occType.equals("P")) {
                occupant = new Predator(occPos, occName, occDesc, scanWikiDescription(input));
                totalPredators++;
